...
export const getTotalPpFromSortedPps = (ppArray: number[], startIdx = 0) =>
	ppArray.reduce(
		(cum: number, pp: number, idx: number) =>
			cum + Math.pow(WEIGHT_COEFFICIENT, idx + startIdx) * pp,
		0,
	);
...
const getTotalPp = (scores: PlayerScore[]) =>
	scores && Array.isArray(scores)
		? getTotalPpFromSortedPps(
				scores.map((s) => s?.score?.pp).sort((a, b) => b - a),
		  )
		: null;

const convertScoresToObject = (
	scores: PlayerScore[],
	idFunc = (score: { leaderboard: { id: any } }) => score?.leaderboard?.id,
	asArray = false,
) =>
...
export function getWhatIfScore(
	scores: PlayerScore[],
	...
	const newTotalPp = getTotalPlayerPp(scores, {
...
export const getPPDifference = async (
	playerID1: string,
	playerID2: string,
