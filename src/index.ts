...
app.get('/toPlayer', async function (req, res) {
	...
	if (req.query.PlayerSSid == undefined || req.query.SSid == undefined) {
		res.send(
			'Please Provide your own scoresaber ID and your targets scoresaber ID in the URL <br> Usage: batthew.co.uk:8081/toPlayer?SSid=1234&targetSSid=10',
		);
	}
	fs.readFile(
		'./html/overlayToPlayer.html',
		'utf-8',
...
app.get('/toNum', function (req, res) {
	...
	if (req.query.SSid == undefined || req.query.num == undefined) {
		res.send(
			'Please provide a scoresaber ID and a target number <br> Usage: batthew.co.uk:8081/toNum?SSid=1234&num=10',
		);
		return;
	}
...
app.get('/plusOne', function (req, res) {
	...
	if (req.query.SSid == undefined) {
		res.send(
			'Please provide a scoresaber ID <br> Usage: batthew.co.uk:8081/plusOne?SSid=1234',
		);
		return;
	}
...
app.get('/ppToNum', async (req: any, res: { send: (arg0: string) => void }) => {
	res.send(
		(
			await diffToTopX(req.query.SSid, req.query.num, req.query.region)
		).toString(),
	);
});
...
app.get(
	...
	async (req: any, res: { send: (arg0: string) => void }) => {
		res.send(
			(await getPPDifference(req.query.SSid, req.query.targetSSid)).toString(),
		);
	},
);
